...
def FutureProphetFifteenMinute():
    ...
    f = open('D:/CRYPTONALYTICS/TrainingModel/predict/fifteenminute/predict.txt',"w")
    for _i in range(len(symbol)):
        data = future_symbol_15min_data(symbol[_i])

        if data is not None:
            prophet = NeuralProphet(seasonality_mode='multiplicative', n_forecasts=4 * 2, epochs=10)
