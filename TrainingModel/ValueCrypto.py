...
class PositionDecisioner(nn.Module):
    def __init__(self, incode, device):
        ...
        self.incode = incode
        self.device = device
        self.tencoder = Transformer(incode, 60, 1, 128, 8, 4, 0.1)
        self.q = nn.Linear(60, 3)
        self.determiner = nn.Linear(60, 2)
        self.leverage = nn.Linear(60, 20)
    ...
    def train_model_p(self, s, s_prime, a, prob_a, reward, gamma=0.98, tau=0.97, weight=0.87,
                      eps_clip=0.1):
        for k in range(10):
    ...
    def train_model_l(self, s, s_prime, a, prob_a, reward, gamma=0.98, tau=0.97, weight=0.87,
                      eps_clip=0.1):
        for k in range(10):
            td_target = reward + gamma * self.value(s_prime)
            value = self.value(s)
    ...
    def train_model_d(self, s, s_prime, a, prob_a, reward, gamma=0.98, tau=0.97, weight=0.87,
                      eps_clip=0.1):
        for k in range(3):
            td_target = reward + gamma * self.value(s_prime)
            value = self.value(s)
